...
try{
 buildSync({entryPoints:['src/lib/cookies.ts'],outfile:join(out,'cookies.cjs'),bundle:true,platform:'node',format:'cjs',define:{'import.meta.env.BASE_URL':'"/homnayangi/"'}});
 ...
 test('Unicode preferences roundtrip with scoped secure cookies',()=>{api.writeCookie('pool',{name:'Bún chả',disabled:[1,2]});assert.deepEqual(api.readCookie('pool'),{name:'Bún chả',disabled:[1,2]});assert.match(writes.at(-1),/Path=\/homnayangi\/; Max-Age=31536000; SameSite=Lax; Secure/);assert.doesNotMatch(writes.at(-1),/Domain=/)});
