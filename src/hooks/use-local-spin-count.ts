...
export function useLocalSpinCount(){
 ...
 const recordSpin=useCallback((food:Food)=>{
  ...
  try{writeCookie('spins',next);writeCookie('last-choice',{name:food.name,price:food.price,veg:!!food.veg,at:Date.now()})}catch{/* Choosing a dish remains available when cookies are disabled. */}
