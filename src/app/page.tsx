...
import { amountFromInput, amountStep, amountToInput, copy, foodName, foodSubtitle, priceLabel, spendRangeHint, SPEND_MAX, SPEND_MIN, type Language } from '@/lib/i18n';
import { useLocalSpinCount } from '@/hooks/use-local-spin-count';
import { PreferencesPanel } from '@/components/preferences-panel';
import { usePreferences } from '@/hooks/use-preferences';
import { personalFoods, personalSelector, withinSpend } from '@/lib/personal-pool';
// Ceilings in thousands of VND, round in the currency the catalog is priced in
// and reading as $10/$12/$15/$18/$23/$32 in English. The last one clears every
// catalog dish; only a custom dish can sit above it. The saved cookie is
// validated against this same list, so a preset cannot be added in one place
// and silently rejected in the other.
const SPEND_PRESETS=['35','50','75','100','150','260'];
// Default: high enough that a full case is mostly open rather than filtered
// down to the counter dishes on a visitor's first spin.
...
// Read before the first render: the spend input is typed in the language's own
// currency, so a later correction would reinterpret what is already on screen.
function readLanguage():Language{try{const saved=readCookie<string>('language');return saved==='en'?'en':'vi'}catch{return 'vi'}}
...
export default function Home(){
 ...
 const [budget,setBudget]=useState(DEFAULT_SPEND),[custom,setCustom]=useState(()=>amountToInput(Number(DEFAULT_SPEND),readLanguage())),[veg,setVeg]=useState(false),[sound,setSound]=useState(true),[spinning,setSpinning]=useState(false),[result,setResult]=useState<Food|null>(null),[revealed,setRevealed]=useState(false);
 ...
 useEffect(()=>{document.documentElement.lang=language;document.title=language==='en'?'What should I eat?':'Ăn gì?'},[language]);
 // Carry the typed spend across the switch as the same money, not the same digits.
 const changeLanguage=(next:Language)=>{setLanguage(next);setCustom(text=>{const thousands=amountFromInput(text,language);return Number.isFinite(thousands)?amountToInput(thousands,next):text});try{writeCookie('language',next)}catch{}};

 const [preferencesReady,setPreferencesReady]=useState(false);
 const [cookieError,setCookieError]=useState('');
 useEffect(()=>{const saved=readCookie<Record<string,unknown>>('settings');if(saved&&typeof saved==='object'){if(typeof saved.budget==='string'&&[...SPEND_PRESETS,'custom'].includes(saved.budget))setBudget(saved.budget);if(typeof saved.custom==='string'&&Number(saved.custom)>=SPEND_MIN&&Number(saved.custom)<=SPEND_MAX)setCustom(amountToInput(Number(saved.custom),language));if(typeof saved.veg==='boolean')setVeg(saved.veg);if(typeof saved.sound==='boolean')setSound(saved.sound)}setPreferencesReady(true)},[]);
 // The cookie keeps thousands of VND whatever currency the input is showing.
 const customThousands=amountFromInput(custom,language);
 useEffect(()=>{if(preferencesReady){try{writeCookie('settings',{budget,custom:String(customThousands),veg,sound});setCookieError('')}catch{setCookieError(language==='vi'?'Không thể lưu cookie. Lựa chọn chỉ giữ trong lần mở trang này.':'Cookies unavailable. Preferences last only for this visit.')}}},[preferencesReady,budget,customThousands,veg,sound,language]);
 const target=budget==='custom'?customThousands:Number(budget);
 const validTarget=Number.isInteger(target)&&target>=SPEND_MIN&&target<=SPEND_MAX;
 ...
 // A spend the visitor is still typing must not empty the case under them, so
 // an out-of-range value falls back to the ceiling; the open button is disabled
 // either way until it is valid again.
 const cap=validTarget?target:SPEND_MAX;
 // Kept apart from `eligible` so an empty case can say which filter emptied it.
 const vegPool=useMemo(()=>population.filter(f=>!veg||f.veg),[population,veg]);
 const eligible=useMemo(()=>withinSpend(vegPool,cap),[vegPool,cap]);
 const lunchSelector=useMemo(()=>personalSelector(eligible,cap),[eligible,cap]);
 ...
 {/* The reel would otherwise advertise catalog dishes the visitor never added. */}
 {preferences.ready&&!population.length
  ? <div className="empty-case"><Utensils size={42} aria-hidden="true"/><strong>{t.emptyCase}</strong><p>{t.emptyCaseHint}</p><PreferencesPanel preferences={preferences} language={language} disabled={spinning} variant="inventory"/></div>
 ...
 <div className="control-bar"><div className="filters"><div className="budget"><label id="budget-label">{t.spend}</label><Select value={budget} onValueChange={v=>setBudget(v??DEFAULT_SPEND)} disabled={spinning}><SelectTrigger aria-labelledby="budget-label"><SelectValue>{budget==='custom'?t.custom:priceLabel(budget,language)}</SelectValue></SelectTrigger><SelectContent>{SPEND_PRESETS.map(v=><SelectItem key={v} value={v}>{priceLabel(v,language)}</SelectItem>)}<SelectItem value="custom">{t.custom}</SelectItem></SelectContent></Select>{budget==='custom'&&<div className="custom-spend"><input aria-label={t.customSpend} aria-invalid={!validTarget} type="number" inputMode={language==='en'?'decimal':'numeric'} min={amountToInput(SPEND_MIN,language)} max={amountToInput(SPEND_MAX,language)} step={amountStep(language)} value={custom} disabled={spinning} onChange={e=>setCustom(e.target.value)}/><span>{t.spendUnit}</span></div>}{!validTarget&&<small className="spend-note" role="alert">{spendRangeHint(language)}</small>}{validTarget&&eligible.length>0&&<small className="spend-note">{t.vegetarianPool} {priceLabel(Math.round(filteredMean),language,true)} / {language==='vi'?'bữa':'meal'}</small>}</div><label className="veg"><Switch checked={veg} onCheckedChange={setVeg} disabled={spinning} aria-label={t.vegetarianOnly}/><span><Leaf size={15}/> {t.vegetarian}</span></label></div><div className="open-wrap"><button className="open-button" disabled={spinning||!validTarget||!eligible.length} onClick={open}>{spinning?<AudioLines size={22}/>:<Sparkles size={21}/>} {spinning?t.opening:result?t.openAgain:t.open} <span>↗</span></button></div></div>
 ...
 </><footer><span>Ăn Gì · <a href={`${basePath}/privacy.html`}>{language==='vi'?'Quyền riêng tư':'Privacy'}</a> · <a href={`${basePath}/terms.html`}>{language==='vi'?'Điều khoản':'Terms'}</a></span><span>{t.footer} <a href="https://github.com/sourcesounds/csgo" target="_blank" rel="noreferrer">SourceSounds</a></span></footer>
 </main></div>
}
